...
_JSON_CT = [[b"content-type", b"application/json"]]
...
async def app(scope, receive, send):
    ...
    if path == "/api/status":
        ...
        await send({"type": "http.response.start", "status": 200, "headers": _JSON_CT})
        await send({"type": "http.response.body", "body": body})
    elif path == "/api/runs" and scope.get("method") == b"POST":
        body = json.dumps({"accepted": True}).encode()
        await send({"type": "http.response.start", "status": 202, "headers": _JSON_CT})
        await send({"type": "http.response.body", "body": body})
